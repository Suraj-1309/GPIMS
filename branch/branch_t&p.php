...
<body>
    ...
    <div class="p-3 ml-5 pl-4">
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            // 1. Edit Functionality (Modal Edit) - Update Current Condition
            if (isset($_POST['snoEdit'])) {
                // Retrieve and trim the current condition input
                $snoEdit = $_POST['snoEdit'];
                $current_condition = trim($_POST['current_conditionEdit']);

                // Validate that the current condition does not exceed 50 characters
                if (strlen($current_condition) > 50) {
                    $_SESSION['popup_message'] = "Current condition must be 50 characters or less.";
                    $_SESSION['popup_type'] = "danger";
                    header("Location: " . $_SERVER['PHP_SELF']);
                    exit();
                }

                // Update the branch_items table with the new current condition
                $updateQuery = "UPDATE branch_items SET current_condition = '$current_condition' WHERE sno = '$snoEdit'";
                $updateResult = mysqli_query($conn, $updateQuery);

                if ($updateResult) {
                    $_SESSION['popup_message'] = "Current condition updated successfully.";
                    $_SESSION['popup_type'] = "success";
                } else {
                    $_SESSION['popup_message'] = "Failed to update record: " . mysqli_error($conn);
                    $_SESSION['popup_type'] = "danger";
                }
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="editForm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                            ...
                            <div class="form-group">
                                <label for="current_conditionEdit">Current Condition</label>
                                <textarea class="form-control" id="current_conditionEdit" name="current_conditionEdit"
                                    rows="2" maxlength="50" required></textarea>
                            </div>
            ...
            <table class="table table-bordered" id="myTable">
                <thead>
                    <tr>
                        ...
                        <th scope="col">Corrent Condition</th>
                        <th scope="col">Action</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    $sql = "SELECT * FROM `branch_items` WHERE `branch` = '$_SESSION[branch]' AND `lab` = '$_SESSION[lab]' AND `type` = 'T&P Item' ORDER BY `sno` DESC";
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            Array.from(editButtons).forEach(function (button) {
                button.addEventListener('click', function (e) {
                    ...
                    // Get the row and its <td> cells (ignoring the <th> cell)
                    let tr = e.target.closest('tr');
                    let tds = tr.getElementsByTagName('td');

                    // Based on your table:
                    // tds[0]: Product Name, tds[1]: Stock Reg Page/Sno,
                    // tds[2]: Allotment Date, tds[3]: Units, tds[4]: Current Condition
                    let product_name = tds[0].innerText;
                    let rr_reg = tds[1].innerText;
                    let allotment_date = tds[2].innerText;
                    let current_condition = tds[4].innerText;
                    ...
                    document.getElementById('current_conditionEdit').value = current_condition;
